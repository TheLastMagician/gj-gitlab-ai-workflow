...
本项目把 `docs/archive/initial-workflow-design.md` 中的方案整理成可复用
...
当前 Issue / MR
  ...
  -> 代码变更读取 docs/standards/01-development-standard.md 和 07-test-standard.md
  -> changed paths 命中的 API / 数据库 / 安全 / 环境专项规范
...
- `gj-codebase-map` 根据现有代码起草开发/测试规范、当前上下文、模块文档和风险 Issue，
  不保存中间扫描报告。
...
早期详细设计背景见 `docs/archive/initial-workflow-design.md`；当前可执行流程以本文、
