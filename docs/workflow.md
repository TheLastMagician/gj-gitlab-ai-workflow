...
8. 重要里程碑结束时生成 `ai-context-summary.md`;普通 Fast 和多数单 Issue 不建迭代目录。
...
13. Milestone 是目标版本，Git Tag 才是已发布版本；普通功能 MR 不 bump 版本。
...
`.gj/context.yml` 渐进加载，不扫描整个 `docs/`，不默认读取历史迭代。
各 Skill 在当前改动内完成对应文档回写，`gj-close-loop` 修剪上下文并只登记最新一轮
摘要。完整分层、预算、文件边界和读写算法见 `docs/documentation-governance.md`；安装到
