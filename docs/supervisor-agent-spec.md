...
4. **临期 milestone**:active milestone 距 due date ≤ 3 天,
   列出未关闭 issue 数和清单。
5. **版本回写欠账**:`docs/releases/<tag>.md` 状态为 `released`,但
   `docs/context/current-state.md` 未记录同一 Tag、commit SHA 或实际部署环境
   → 提示发布事实尚未完成回写。Milestone 和迭代目录不能单独证明已经发布。
...
- 角色与 owner 名单:`.gj/workflow.yml` 的 `roles` 段。
- 高风险路径:`.gj/workflow.yml`(检查 6 可用它决定优先扫哪些模块)。
