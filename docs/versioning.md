...
3. `gj-plan-change` 把 `Target release: v1.3.0` 写入功能文档。
...
| 载体 | 必须关联 |
| --- | --- |
| Requirement Issue | flow、Target release、GitLab Milestone、仓库文档链接 |
| PRD/设计/方案/测试计划 | Source Issue、Target release、直接上下游文档 |
| MR | Issue、Milestone、文档决策；默认不修改版本 |
| 测试报告 | Version、Tag、commit、Pipeline、环境、Issues/MRs、证据 |
| 发布说明 | Version、Milestone、Tag、commit、Pipeline、测试报告、包含项、回滚 |
| current state | 最新仓库发布版本和各环境实际部署的 Tag/SHA |

功能文档按语义命名并原地更新，例如 `order-manager-approval.md`；不要为版本复制 PRD、
...
- Tag 符合配置的 SemVer 格式；
- 对应发布说明存在；
- 发布说明 `Version`、`Tag` 与实际 Tag 一致，状态为 `ready` 或 `released`；
- 包含项、验证和回滚章节存在。
