...
| 名称 | 事实源 | 生命周期 |
| --- | --- | --- |
| Target release | GitLab Milestone | 需求计划时确认，可在发布前调整 |
...
| 阶段 | 版本动作 | 文档动作 |
| --- | --- | --- |
| 入口 | 人确认目标版本/Milestone | Requirement Issue 记录 Target release 和文档链接 |
| 计划 | 版本仍是计划，可调整 | PRD、设计、方案、测试计划记录 Source Issue 和 Target release |
...
| 文档 | 最低版本字段 |
| --- | --- |
| PRD/设计/方案/测试计划 | Source Issue、Target release、直接上下游链接 |
| 测试报告 | Version、Tag、commit、Pipeline、Environment、Related Issues/MRs |
| 发布说明 | Version、Milestone、Tag、commit、Pipeline、Test report、Status |
| current state | Latest released Tag、各环境 deployed Tag/SHA、最近验证时间 |
...
- Tag Pipeline 的 `release_version_check.py` 硬检查 Tag 格式、发布说明存在性、Version、
  Tag、Status 和必要章节，避免错误版本进入构建/部署。
