...
| --- | --- | --- | --- | --- |
| 入口分流 | `gj-workflow-next` | Standard 创建 Requirement Issue；Hotfix 创建 Hotfix Issue；Fast 按需创建 SmallChange Issue | 不默认写仓库文档，只列预期文档影响 | flow、Target release、主工作项由 PdM/PM 确认 |
...
| 文档类型 | 真实文档路径 | 命名 |
| --- | --- | --- |
| PRD | `docs/product/requirements/<capability>.md` | 稳定能力名，如 `order-approval.md` |
...
- `Status` 只使用 `draft` 或 `confirmed`。确认时记录确认人和日期。
- `Effective from` 表示从哪个 Tag 或仓库状态开始适用；未发布时允许写 `pending`。
- `Implemented by` 链接实际 MR；计划阶段允许留空，开发阶段补齐。
- `Last verified` 是最后一次对照代码或业务确认的日期，不等于普通编辑时间。
...
| API 契约 | 权威 schema 位置、请求/响应、错误、权限、幂等、兼容和回归要求 | API Owner/Dev Lead |
| 数据库设计 | 权威 schema/migration、实体关系、字段语义、约束、索引、数据保留、迁移和回滚 | Data Owner/Dev Lead |
| ADR | 背景、决策、备选项、取舍和后果 | Dev Lead；确认后冻结 |
| 模块文档 | 职责边界、业务规则、状态/失败行为、接口和数据契约、依赖、测试入口 | Module Owner |
...
文档决策必须同时列出可执行路径和解释文档路径；路径未定时写 `TBD`、Owner 和确认期限。

## 8. 创建、更新、冻结和删除

1. 没有持久事实变化：不建文档，文档决策写 `no-change`。
2. 已有同一能力、模块或契约边界：原地更新为当前完整事实。
3. 新的稳定能力、模块或独立评审边界：从 `.gj/doc-templates/` 复制合适模板并使用
   语义文件名创建。
4. ADR：新的不可逆或跨团队技术取舍才新增；确认后不改写，后续用新 ADR 取代。
5. 测试报告和发布说明：按 Tag 新建，关闭后冻结。
6. 当前事实失效：直接修改或删除；历史由 Git、Issue/MR 和冻结证据追溯。
7. 文档路径或模块边界变化：同步更新 `.gj/context.yml` 和直接上下游链接。
...
| --- | --- | --- | --- | --- |
| docs/modules/order.md | update | 金额审批规则变化 | develop / confirmed | @module-owner |
| docs/technical/apis/order.md | no-change | API 契约未变化 | develop / - | - |
```

动作只能是 `create`、`update`、`no-change` 或 `follow-up`。每一种受影响的持久事实都要
有一行；`follow-up` 必须包含 GitLab Issue、负责人和期限。Reviewer 逐项对照实际 diff、
行为变化、确认记录和 Target release。
