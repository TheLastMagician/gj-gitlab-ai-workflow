...
|---|---|---|---|---|
| Explore | 需求 Issue 评论 | `gj-plan-change` | 澄清问题、业务目标、边界 | 需求能被业务确认 |
| Spec | 需求 Issue | `gj-plan-change` | 验收标准、非目标范围、DoR | 需求已确认 |
| Plan | 方案 Issue | `gj-plan-change` | 技术方案、风险、影响范围 | Tech Lead 确认 |
| Split | 开发 / 测试 / 发布 Issue | `gj-plan-change` | 任务清单、依赖、优先级 | 可进入排期 |
| Execute | 分支 + MR | `gj-develop-change` | 实现建议、上下文摘要 | MR 创建 |
| Review | MR | `gj-mr-review` | 变更摘要、风险、测试建议 | Pipeline 通过、线程解决 |
| Verify | 测试 Issue | `gj-plan-change` | 测试用例、回归范围、结果 | 测试 / 验收通过 |
| Release | 发布 Issue / tag | `gj-release-readiness` | Release Note、回滚、验证 | 发布完成 |
| Retro | 复盘 Issue | `gj-close-loop` | 复盘报告、改进项 | 改进项明确 |
| Context Update | 仓库文档 | `gj-close-loop` | `ai-context-summary.md`、模块文档、ADR | 长期上下文已更新 |
...
```text
使用 gj-close-loop 总结本 Milestone，并生成 ai-context-summary.md。
```
