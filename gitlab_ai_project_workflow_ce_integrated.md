...
复盘
  ↓
更新 docs/iterations、ai-context-summary、当前上下文、模块文档和 ADR
```
...
| 需求 Issue | 业务需求、范围、验收标准。 |
| 方案 Issue | 技术方案、影响范围、风险和评审结论。 |
| 开发任务 Issue | 后端、前端、数据库、文档、运维等可执行任务。 |
| 测试 Issue | 测试范围、用例、结果、缺陷跟踪。 |
...
|---|---|---|---|---|
| Explore | 需求 Issue 评论 | `gj-requirement-refine` | 澄清问题、业务目标、边界 | 需求能被业务确认 |
| Spec | 需求 Issue | `gj-requirement-refine` | 验收标准、非目标范围、DoR | 需求已确认 |
| Plan | 方案 Issue | `gj-solution-plan` | 技术方案、风险、影响范围 | Tech Lead 确认 |
| Split | 开发 / 测试 / 发布 Issue | `gj-issue-split` | 任务清单、依赖、优先级 | 可进入排期 |
| Execute | 分支 + MR | `gj-dev-context` | 实现建议、上下文摘要 | MR 创建 |
| Review | MR | `gj-mr-review` | 变更摘要、风险、测试建议 | Pipeline 通过、线程解决 |
| Verify | 测试 Issue | `gj-test-design` | 测试用例、回归范围、结果 | 测试 / 验收通过 |
| Release | 发布 Issue / tag | `gj-release-prep` | Release Note、回滚、验证 | 发布完成 |
| Retro | 复盘 Issue | `gj-retro-learnings` | 复盘报告、改进项 | 改进项明确 |
| Context Update | 仓库文档 | `gj-context-extract` | `ai-context-summary.md`、模块文档、ADR | 长期上下文已更新 |

# 6. 流程分流规则

本工作流约束的是交付节点、输入输出、流转条件和沉淀要求，不约束个人在节点内使用的具体工具和方法。产品、开发、测试、运维可以使用任意工具或 AI，只要完成当前节点产物，并满足进入下一节点的条件。

不是所有事项都需要走完整重流程。先由人或 `gj-workflow-triage` 判断事项类型，再选择对应路径。

| 类型 | 适用场景 | 可跳过 | 必须保留 |
|---|---|---|---|
| 标准需求 | 新功能、复杂改造、跨模块、影响业务规则 | 无 | 需求确认、方案、任务、MR、测试、发布、复盘 |
| 小改动 | 文案、样式、小字段、低风险局部逻辑 | 方案评审、任务拆分、完整迭代文档 | 轻量 Issue、MR、CI、Review、自测 |
...
如果 Bug 修复过程中发现需求理解、业务规则或架构设计存在问题，应升级为标准需求或补充方案 Issue。
...
3. 回归结果。
4. 是否需要补测试。
5. 是否需要更新 `docs/context/`、`docs/modules/`、ADR 或 `.ai/context-index.yml`。
6. 是否需要调整流程或门禁。

## 6.5 分流判断规则

`gj-workflow-triage` 应读取 Issue 标题、描述、标签、影响模块和风险关键词，输出建议路径：
...
命中模块的有效 ADR
  ↓
最近相关迭代的 ai-context-summary.md
  ↓
必要时追溯完整 docs/iterations/*
```
...
| 技术决策 | `docs/architecture/adr/*.md` | 仍然有效的架构决策。 |
| 迭代摘要 | `docs/iterations/*/ai-context-summary.md` | 历史迭代给 AI 的短摘要。 |
| 历史追溯 | `docs/iterations/*` | 需要时再读取完整记录。 |
...
2. 历史迭代默认只读摘要。
3. 旧方案不能直接当作当前事实。
4. 如果当前事实和历史记录冲突，AI 必须提示人工确认。
5. 发布或复盘后，长期有效变化必须同步到当前上下文或模块文档。

# 12. 迭代知识沉淀

每个 Milestone 或版本建立一个迭代目录：

```text
docs/iterations/<yyyy-mm>-<version>-<short-name>/
```
...
| 文件 | 内容 |
|---|---|
| `00-index.md` | 迭代目标、范围、Milestone、总控 Issue、相关链接。 |
| `01-requirements.md` | 需求摘要、验收标准、关键澄清。 |
| `02-solution.md` | 技术方案摘要、影响范围、重要取舍。 |
| `03-task-map.md` | 开发、测试、发布任务和依赖。 |
| `04-test-report.md` | 测试范围、结果、缺陷、回归结论。 |
| `05-release.md` | 发布内容、配置、DB 变更、回滚和验证。 |
| `06-retro.md` | 复盘结论、问题、改进项。 |
| `ai-context-summary.md` | 后续 AI 读取的短摘要。 |

`ai-context-summary.md` 推荐结构：
...
| 变化类型 | 更新位置 |
|---|---|
| 当前业务规则变化 | `docs/context/current-state.md`、相关 `docs/modules/*.md` |
| 模块边界变化 | `docs/context/module-map.md`、`.ai/context-index.yml` |
| 新术语 | `docs/context/glossary.md` |
| 架构或长期技术决策 | `docs/architecture/adr/*.md` |
| 风险路径或负责人变化 | `.ai/rule-map.yml` |
...
增量刷新只更新命中路径相关的 codebase 文档、模块文档和 `context-index`，避免每次重新扫描整个项目。
...
4. 既有项目第一次接入时，`docs/context/` 和 `.ai/context-index.yml` 应从 codebase map 派生，而不是凭空编写。
...
```text
使用 gj-codebase-map 梳理这个已有项目，并生成 docs/codebase、docs/context、docs/modules 和 context-index 草案。
...
3. 已有 `docs/codebase/`、`docs/context/`、`docs/modules/`、ADR 和 `.ai/context-index.yml`。
...
11. `.ai/context-index.yml` 草案或更新建议
...
5. 已给出 `docs/context/`、`docs/modules/` 和 `.ai/context-index.yml` 的更新建议。
...
context:
  index: ".ai/context-index.yml"
  current_state: "docs/context/current-state.md"
  module_map: "docs/context/module-map.md"
  glossary: "docs/context/glossary.md"
  iterations_root: "docs/iterations"
  modules_root: "docs/modules"
  adr_root: "docs/architecture/adr"
  load_full_history_by_default: false

security:
  redact_secrets: true
  store_prompt: false
  store_ai_response: true
```

## 14.2 `.ai/rule-map.yml`

用于把变更路径映射到规范、风险和负责人确认。
...
## 14.3 `.ai/context-index.yml`

用于把变更路径映射到模块背景、ADR 和迭代摘要。
...
modules:
  order:
    ...
    active_decisions:
      - "docs/architecture/adr/0001-order-state-machine.md"
    recent_iteration_summaries:
      - "docs/iterations/2026-07-v1.1-order-approval/ai-context-summary.md"
    ai_focus:
      - "订单状态机是否兼容历史状态"
      - "审批流是否影响查询、导出和权限"
...
iteration_policy:
  ...
  update_current_state_on_release: true
  max_recent_summaries_per_module: 3
```

# 15. Skills 设计

后续建议把各节点封装成 skills。每个 skill 只负责一个清晰场景，详细规则放入 references，重复 API 操作放入 scripts。
...
| `gj-codebase-map` | 梳理既有项目，生成 codebase map、当前上下文、模块文档和 context-index 草案。 |
| `gj-iteration-bootstrap` | 创建新迭代目录、Milestone、项目总控 Issue。 |
| `gj-workflow-triage` | 判断事项走标准需求、小改动、Bug 修复还是 Hotfix。 |
| `gj-requirement-refine` | 需求澄清、验收标准、DoR 检查。 |
| `gj-solution-plan` | 技术方案草案和风险分析。 |
| `gj-issue-split` | 拆分开发、测试、发布任务。 |
| `gj-dev-context` | 开发前读取 Issue、方案、规范和项目背景。 |
| `gj-bug-fix` | 缺陷定位、根因分析、修复建议和回归范围。 |
| `gj-hotfix` | 紧急修复路径、最小必要确认、发布验证和事后补齐项。 |
| `gj-mr-review` | MR 摘要、风险、规范命中、测试建议。 |
| `gj-test-design` | 测试用例和回归范围。 |
| `gj-release-prep` | 发布说明、回滚、上线验证。 |
| `gj-retro-learnings` | 复盘和经验沉淀。 |
| `gj-context-extract` | 从完成迭代中提炼长期上下文。 |
| `gj-workflow-next` | 判断当前项目下一步该做什么。 |
...
4. 根据 `.ai/rule-map.yml` 和 `.ai/context-index.yml` 装载规范和背景。
5. 调用 AI Gateway 或模型服务。
6. 把结果写回 GitLab 评论。
7. 维护状态标签互斥。
8. 识别高风险路径并提示 owner ack。
9. 生成迭代摘要和复盘上下文。
10. 记录审计日志。
...
| Milestone closed | 有复盘 Issue | 生成复盘和上下文摘要 |
...
5. MR 是否填写回滚方案。
6. 是否声明 AI 使用情况。
7. 命中高风险路径时，是否存在 `/owner-ack` 评论。
8. 是否存在明显 secret、token、password。
...
6. 流程改进建议。
7. 需要进入长期上下文的变化。
8. 需要更新的模块文档、ADR、`context-index`。
9. 本迭代 `ai-context-summary.md`。
...
| 指标 | 含义 |
|---|---|
| 迭代目录完整率 | 每个 Milestone 是否有固定迭代目录 |
| `ai-context-summary.md` 覆盖率 | 每个已发布迭代是否有 AI 摘要 |
| 当前上下文更新率 | 长期有效变化是否同步到当前事实 / 模块文档 |
| `context-index` 命中率 | AI 是否能按路径找到相关背景 |
...
4. 建立 `docs/iterations/`。
5. 建立 `docs/architecture/adr/`。
6. 配置 `.ai/project.yml`。
7. 配置 `.ai/rule-map.yml`。
8. 配置 `.ai/context-index.yml`。
...
4. 支持 `/ai-triage`、`/ai-analyze`、`/ai-review-mr`、`/ai-bug-fix`、`/ai-hotfix`、`/ai-retro` 等命令。
5. 把 AI 输出写回 GitLab 评论。
6. 加入审计日志和脱敏。

## 21.4 第四阶段：Skills 化

1. 先封装 `gj-workflow-bootstrap` 和 `gj-codebase-map`。
2. 再封装分流、需求、MR Review、Bug 修复、Hotfix、复盘几个高频 skill。
3. 每个 skill 固定输入、输出和 GitLab 写回格式。
4. 把重复 API 操作沉淀成 scripts。
5. 在团队中试用后再扩展测试、发布、上下文提取等 skills。
...
| 8 | `docs/iterations/` | PM / Tech Lead |
| 9 | `.ai/project.yml` | Tech Lead / AI 平台负责人 |
| 10 | `.ai/rule-map.yml` | Tech Lead / 安全 / DevOps |
| 11 | `.ai/context-index.yml` | Tech Lead / 架构师 |
| 12 | `.gitlab-ci.yml` 和 `policy_check.py` | DevOps |
| 13 | 项目总控 Issue | PM / Tech Lead |
| 14 | Milestone | PM |
...
```text
使用 gj-codebase-map 梳理当前项目，生成 docs/codebase、docs/context、docs/modules 和 .ai/context-index.yml 草案。
```

流程分流：

```text
使用 gj-workflow-triage 判断 #128 应该走标准需求、小改动、Bug 修复还是 Hotfix。
```

产品经理：

```text
使用 gj-requirement-refine 分析 #123，准备和业务讨论的问题。
```

Tech Lead：

```text
使用 gj-solution-plan 基于 #123 生成方案草案和风险清单。
```

开发：

```text
使用 gj-dev-context 读取 #145、关联方案和项目上下文，告诉我实现时要注意什么。
```

Bug 修复：

```text
使用 gj-bug-fix 分析 #156 的复现步骤和日志，给出根因、修复建议和回归范围。
```

紧急修复：

```text
使用 gj-hotfix 处理 #159，输出最小修复路径、发布验证和事后补齐清单。
```
...
```text
使用 gj-test-design 根据需求、方案和 MR 生成测试用例。
```

项目负责人：

```text
使用 gj-retro-learnings 总结本 Milestone，并生成 ai-context-summary.md。
```
...
5. `gj-workflow-triage` 能判断标准需求、小改动、Bug 修复、Hotfix。
6. 需求分析 AI 评论。
7. Bug 修复 AI 建议。
8. MR Review AI 评论。
9. 复盘生成 `ai-context-summary.md`。
10. `.ai/context-index.yml` 能让 AI 读到相关模块背景。
