...
核心目标不是让 AI 自主审批、合并或发布，而是让团队里的产品经理（产品）、技术负责人
（技术）、开发工程师（开发）、代码审阅人（审阅）、测试负责人（测试）、环境发布负责人
（发布）、项目经理/迭代负责人（项目）都可以用 AI 辅助完成自己负责的动作。最终责任和
确认权仍然在人手里。
...
README 里的负责人统一使用“中文名称（简称）”。简称用于流程图、表格和 GitLab 交接评论，
避免在日常流转里混用英文缩写。

| 统一称呼 | 主要责任 |
| --- | --- |
| 产品经理（产品） | 负责需求来源、业务目标、验收标准、非目标、产品规则、原型/交互说明。 |
| 项目经理/迭代负责人（项目） | 负责推进节奏、排期、风险、跨角色协调、复盘和待办闭环。 |
| 技术负责人（技术） | 负责技术方案、架构判断、任务拆分、技术风险、技术评审口径。 |
| 开发工程师（开发） | 负责编码、单测、自测、提交 MR、修复审阅或测试发现的问题。 |
| 代码审阅人（审阅） | 负责审 MR，检查代码质量、风险路径、测试覆盖和文档影响。 |
| 合并负责人（合并） | 负责最终合并判断和合并操作，通常需要 GitLab 维护者权限。 |
| 测试负责人（测试） | 负责测试计划、验收测试、回归测试、测试报告、缺陷跟踪。 |
| 环境发布负责人（发布） | 负责 CI/CD、环境部署、共享测试环境锁、发布准备、回滚方案。 |
| 任意成员（个人） | 通过 `gj-workflow-inbox` 查看分配给自己的 GitLab 待办。 |
...
```mermaid
flowchart TD
  A["安装本仓库 Codex skills"] --> B["在目标业务项目安装 workflow 资产"]
  B --> C["项目初始化：gj-workflow-bootstrap"]
  C --> D["填写 .ai/project.yml / role-map / rule-map"]
  D --> E{"已有代码项目？"}
  E -- "是" --> F["代码上下文梳理：gj-codebase-map"]
  E -- "否" --> G["配置 GitLab labels / 保护分支 / Runner"]
  F --> G
  G --> H["验证 workflow 资产和角色映射"]
  H --> I["开始用 gj-workflow-inbox 接收个人待办"]
```
...
- 填写 `.ai/role-map.yml`，把产品经理（产品）、项目经理/迭代负责人（项目）、
  技术负责人（技术）、开发工程师（开发）、代码审阅人（审阅）、合并负责人（合并）、
  测试负责人（测试）、环境发布负责人（发布）映射到真实 GitLab 用户。
...
```mermaid
flowchart TD
  A["个人：gj-workflow-inbox<br/>读取 GitLab 待办"] --> B["产品/项目：gj-workflow-triage<br/>需求分流"]
  B --> C{"工作类型"}

  C -- "标准需求" --> D["产品：gj-requirement-refine<br/>澄清需求和验收标准"]
  D --> E["技术：gj-solution-plan<br/>技术方案和风险评估"]
  E --> F["技术/项目：gj-issue-split<br/>拆分开发/测试/发布/文档任务"]

  F --> G["开发：gj-dev-context<br/>读取上下文并实现"]
  F --> H["测试：gj-test-design<br/>设计验收和回归用例"]
  G --> I["开发：提交 MR<br/>代码、测试、文档影响"]
  I --> J["审阅：gj-mr-review<br/>审代码和风险"]
  J --> K{"审阅通过？"}
  K -- "否" --> G
  K -- "是" --> L["发布/测试：gj-env-deploy-assist<br/>部署 dev/review 或申请共享 test"]
  H --> M["测试：执行测试<br/>记录测试报告"]
  L --> M
  M --> N{"发现缺陷？"}
  N -- "是" --> O["开发/测试：gj-bug-fix<br/>定位、修复、回归"]
  O --> I
  N -- "否" --> P["合并：gj-merge-assist<br/>人工授权后合并"]
  P --> Q["发布/项目：gj-release-prep<br/>发布说明、上线检查、回滚方案"]
  Q --> R["项目/技术：gj-retro-learnings<br/>复盘"]
  R --> S["项目/技术：gj-context-extract<br/>沉淀长期 AI 上下文"]

  C -- "小改动" --> G
  C -- "Bug 修复" --> O
  C -- "Hotfix" --> T["发布/技术：gj-hotfix<br/>紧急修复和事后补齐"]
  T --> P
```

| 阶段 | 负责人 | 使用 skill | 产物 |
| --- | --- | --- | --- |
| 待办读取 | 任意成员（个人） | `gj-workflow-inbox` | 当前 GitLab Todo、分配给自己的 Issue/MR、审阅请求、失败 pipeline、下一步建议 |
| 需求分流 | 产品经理（产品）/ 项目经理（项目） | `gj-workflow-triage` | 判断走标准需求、小改动、Bug 修复还是 Hotfix |
| 需求澄清 | 产品经理（产品） | `gj-requirement-refine` | 需求 Issue、验收标准、非目标、风险、文档影响 |
| 方案设计 | 技术负责人（技术） | `gj-solution-plan` | 技术方案、影响范围、测试策略、发布和回滚思路 |
| 任务拆分 | 技术负责人（技术）/ 项目经理（项目） | `gj-issue-split` | 开发、测试、发布、文档、后续事项等可追踪 Issue |
| 开发准备 | 开发工程师（开发） | `gj-dev-context` | 当前 Issue 相关上下文、应读文件、风险点、实现边界 |
| 提交 MR | 开发工程师（开发） | 常规开发 + MR 模板 | 代码、测试、文档影响说明、关联 Issue |
| MR 审阅 | 代码审阅人（审阅） | `gj-mr-review` | 审阅意见、阻塞项、风险路径、是否建议合并 |
| 环境部署 | 环境发布负责人（发布）/ 测试负责人（测试） | `gj-env-deploy-assist` | dev/test 部署计划、环境锁、版本记录、回滚目标 |
| 测试设计与执行 | 测试负责人（测试） | `gj-test-design` | 测试计划、验收用例、回归用例、失败路径、测试报告 |
| 缺陷处理 | 开发工程师（开发）/ 测试负责人（测试） | `gj-bug-fix` | Bug 根因、修复范围、回归验证、修复 MR |
| 合并辅助 | 代码审阅人（审阅）/ 合并负责人（合并） | `gj-merge-assist` | 合并前检查、pipeline 状态、讨论状态、人工授权后的 merge 操作 |
| 发布准备 | 环境发布负责人（发布）/ 项目经理（项目） | `gj-release-prep` | 发布说明、上线检查、回滚方案、发布确认 |
| 复盘沉淀 | 项目经理（项目）/ 技术负责人（技术） | `gj-retro-learnings` | 复盘结论、流程改进、遗留问题 |
| 上下文更新 | 项目经理（项目）/ 技术负责人（技术） | `gj-context-extract` | `ai-context-summary.md`、模块文档、ADR、context index 更新 |
| 下一步判断 | 任意成员（个人） | `gj-workflow-next` | 根据当前 GitLab 状态推荐下一步动作和 skill |
...
| `gj-mr-review` | 代码审阅人审 MR 时，用 AI 辅助看风险、测试、文档影响和遗漏 |
| `gj-merge-assist` | 合并负责人已决定可以合并，需要 AI 做合并前检查或执行被授权的 merge |
| `gj-test-design` | 测试负责人设计验收、回归、权限、失败路径、发布验证用例 |
| `gj-env-deploy-assist` | 需要部署 dev/review/test 环境，尤其是共享测试环境需要环境锁和人工确认 |
| `gj-bug-fix` | 测试负责人或线上发现缺陷，需要复现、定位根因、修复和回归 |
