...
核心目标不是让 AI 自主审批、合并或发布，而是让团队里的产品、Tech Lead、开发、
Reviewer、QA、DevOps、PM 都可以用 AI 辅助完成自己负责的动作。最终责任和确认权
仍然在人手里。
...
- 填写 `.ai/role-map.yml`，把产品、Tech Lead、开发、Reviewer、QA、DevOps、PM
  映射到真实 GitLab 用户。
...
- GitLab MR 的 `reviewer` 是当前 Review 责任人。
- 交接评论必须 `@username`。
...
| 阶段 | 负责人 | 使用 skill | 产物 |
| --- | --- | --- | --- |
| 待办读取 | 每个人 | `gj-workflow-inbox` | 当前 Todo、assigned Issue/MR、review request、失败 pipeline、下一步建议 |
| 需求分流 | 产品 / PM | `gj-workflow-triage` | 判断走标准需求、小改动、Bug 修复还是 Hotfix |
| 需求澄清 | 产品 | `gj-requirement-refine` | 需求 Issue、验收标准、非目标、风险、文档影响 |
| 方案设计 | Tech Lead | `gj-solution-plan` | 技术方案、影响范围、测试策略、发布和回滚思路 |
| 任务拆分 | Tech Lead / PM | `gj-issue-split` | 开发、测试、发布、文档、后续事项等可追踪 Issue |
| 开发准备 | 开发 | `gj-dev-context` | 当前 Issue 相关上下文、应读文件、风险点、实现边界 |
| 提交 MR | 开发 | 常规开发 + MR 模板 | 代码、测试、文档影响说明、关联 Issue |
| MR 审阅 | Reviewer | `gj-mr-review` | Review 意见、阻塞项、风险路径、是否建议合并 |
| 合并辅助 | Reviewer / Maintainer | `gj-merge-assist` | 合并前检查、pipeline 状态、讨论状态、人工授权后的 merge 操作 |
| 测试设计 | QA | `gj-test-design` | 测试计划、验收用例、回归用例、失败路径、测试报告 |
| 缺陷处理 | QA / 开发 | `gj-bug-fix` | Bug 根因、修复范围、回归验证、修复 MR |
| 环境部署 | DevOps / QA | `gj-env-deploy-assist` | dev/test 部署计划、环境锁、版本记录、回滚目标 |
| 发布准备 | DevOps / PM | `gj-release-prep` | 发布说明、上线检查、回滚方案、发布确认 |
| 复盘沉淀 | PM / Tech Lead | `gj-retro-learnings` | 复盘结论、流程改进、遗留问题 |
| 上下文更新 | PM / Tech Lead | `gj-context-extract` | `ai-context-summary.md`、模块文档、ADR、context index 更新 |
| 下一步判断 | 任意角色 | `gj-workflow-next` | 根据当前 GitLab 状态推荐下一步动作和 skill |
...
- 代码必须通过 MR 合并。
- Pipeline 必须成功才能合并。
- AI 可以辅助审批判断、Review、合并检查、发布准备，但不能脱离人的明确授权自主审批、自主合并、自主发布。
...
| `gj-mr-review` | Reviewer 审 MR 时，用 AI 辅助看风险、测试、文档影响和遗漏 |
| `gj-merge-assist` | Maintainer 已决定可以合并，需要 AI 做合并前检查或执行被授权的 merge |
| `gj-test-design` | QA 设计验收、回归、权限、失败路径、发布验证用例 |
| `gj-env-deploy-assist` | 需要部署 dev/review/test 环境，尤其是共享测试环境需要环境锁和人工确认 |
| `gj-bug-fix` | QA 或线上发现缺陷，需要复现、定位根因、修复和回归 |
...
| QA 执行结果、失败项、阻塞项 | `docs/qa/test-reports/<feature>.md` |
...
- `policy`：检查 MR 描述、文档影响、owner 确认、疑似 secret 等。
