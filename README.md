...
项目接入分为“安装工作流”和“理解业务代码”两个阶段，职责不能混用：

| Skill | 负责 | 不负责 |
| --- | --- | --- |
| `gj-workflow-bootstrap` | 安装并校验 `.gj`、GitLab 模板、CI、脚本、文档骨架、flow 标签和角色配置 | 不推断技术栈，不分析模块，不把现有代码习惯写成项目规范 |
| `gj-codebase-map` | 扫描已有代码，起草开发/测试规范、当前状态、模块上下文、AI 路由和风险 Issue | 不安装工作流，不修改 CI、flow 标签、角色或 GitLab 凭据 |

首次接入统一从 `gj-workflow-bootstrap` 开始，用户不需要预先判断该调用哪个 Skill：

```text
空项目：bootstrap 安装并校验工作流 → 初始化完成
已有代码项目：bootstrap 安装并校验工作流 → codebase-map 起草项目规范和上下文
后续重大重构：单独运行 codebase-map 刷新受影响的规范和上下文
```
